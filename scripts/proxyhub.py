...
with tqdm(range(pages)) as pbar:
    for page in pbar:
        ...
        src = requests.get('https://www.proxyhub.me/zh/ar-socks-proxy-list.html', headers=headers).text
        html = BeautifulSoup(src, 'html5lib')
        for item in html.find('tbody').find_all('tr'):
            ip.append(item.find('td').string)
            port.append(item.find('td').next_sibling.string)

os.system("clear")
with ThreadPoolExecutor(len(ip)) as pool:
    with tqdm(range(len(ip))) as pbar:
        tasks = []
